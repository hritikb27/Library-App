...
class UI {
    ...
    static addBookToDisplay(giveBook) {
        ...
        for (let book in giveBook) {
            if (book.includes('status')) {
                const readStatus = this.makeReadStatus(giveBook);
    ...
    static makeReadStatus(giveBook) {
...
submit.addEventListener('click', () => {
    ...
    if (!pages.valueAsNumber) {
        ...
        const book2 = new createBook(bookName.value, author.value, pages.valueAsNumber, serialNumber.valueAsNumber, status.checked);
        Store.addBook(book2)
        form.reset();
    }
